...
let modelChart = null;
let currentTrendMode = '24h';

// 🔍 日志筛选状态
let logFilters = { key_id: '', model: '', status: '' };
...
async function loadDashboard() {
  try {
    ...
    try {
      ...
      const todayTokens = (summary.today.prompt_tokens || 0) + (summary.today.completion_tokens || 0);
      document.getElementById('statTodayTokens').textContent = todayTokens.toLocaleString();
    } catch {}
...
async function loadTrendChart() {
  const days = currentTrendMode === '24h' ? 1 : 7;
  const data = await api(`/api/admin/analytics/trend?days=${days}`);
  ...
  const labels = data.buckets.map(b => {
    if (data.granularity === 'hourly') {
      return b.bucket.split('T')[1] || b.bucket;
    }
    return b.bucket;
  });
  const creditsData = data.buckets.map(b => b.credits || 0);
  const tokensData = data.buckets.map(b => (b.prompt_tokens || 0) + (b.completion_tokens || 0));
  ...
  trendChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: {
          position: 'top',
          labels: { usePointStyle: true, padding: 20, font: { size: 11 }, color: '#64748b' }
        },
        tooltip: {
      ...
      scales: {
        ...
        y: {
          ...
          display: true,
          position: 'left',
          title: { display: true, text: '积分', font: { size: 10 }, color: '#f59e0b' },
          grid: { color: '#f1f5f9' },
        ...
        y1: {
          ...
          display: true,
          position: 'right',
          title: { display: true, text: 'Token', font: { size: 10 }, color: '#6366f1' },
...
function switchTrendMode(mode) {
  ...
  document.getElementById('trendBtn-24h').className =
    mode === '24h' ? 'text-[0.65rem] px-2.5 py-1 rounded-full font-semibold transition-all bg-indigo-100 text-indigo-700'
    : 'text-[0.65rem] px-2.5 py-1 rounded-full font-semibold transition-all text-slate-500 hover:text-indigo-600';
  document.getElementById('trendBtn-7d').className =
    mode === '7d' ? 'text-[0.65rem] px-2.5 py-1 rounded-full font-semibold transition-all bg-indigo-100 text-indigo-700'
    : 'text-[0.65rem] px-2.5 py-1 rounded-full font-semibold transition-all text-slate-500 hover:text-indigo-600';
  loadTrendChart();
}
...
async function loadStatusChart() {
  const data = await api('/api/admin/analytics/status-ratio');
  ...
  const colorMap = { success: '#10b981', failed: '#ef4444', error: '#f59e0b' };
  const labelMap = { success: '成功', failed: '失败', error: '错误' };
  ...
  statusChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: {
          position: 'bottom',
          labels: { usePointStyle: true, padding: 16, font: { size: 11 }, color: '#64748b' }
        },
        tooltip: {
          backgroundColor: '#1e293b',
          callbacks: {
...
async function loadModelChart() {
  const data = await api('/api/admin/analytics/model-usage');
  ...
  const palette = ['#6366f1', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#ec4899', '#14b8a6', '#f97316', '#3b82f6'];
  ...
  modelChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: {
          position: 'bottom',
          labels: { usePointStyle: true, padding: 14, font: { size: 10 }, color: '#64748b' }
        },
        tooltip: {
          backgroundColor: '#1e293b',
          callbacks: {
            label: (ctx) => {
              const total = ctx.dataset.data.reduce((a, b) => a + b, 0);
              const pct = total > 0 ? Math.round(ctx.raw / total * 100) : 0;
              const modelData = data[ctx.dataIndex];
              return [
                ` 调用: ${ctx.raw} 次 (${pct}%)`,
                ` 积分: ${modelData.total_credits}`
              ];
            }
          }
        }
...
async function loadTopConsumers() {
  const data = await api('/api/admin/analytics/top-consumers?limit=10');
  const tbody = document.getElementById('topConsumersBody');
  if (!tbody) return;

  tbody.innerHTML = data.map((c, i) => {
    const successRate = c.total_calls > 0 ? Math.round(c.success_calls / c.total_calls * 100) : 0;
    const rateClass = successRate >= 95 ? 'text-emerald-600' : successRate >= 80 ? 'text-amber-600' : 'text-red-500';
    return `
      <tr class="hover:bg-slate-50 transition-colors">
        <td class="px-4 py-2 text-slate-400 font-bold">${i + 1}</td>
        <td class="px-4 py-2 font-medium text-slate-700">${esc(c.key_name || c.key_value)}</td>
        <td class="px-4 py-2 text-right font-bold text-orange-600 font-mono">${c.total_credits}</td>
        <td class="px-4 py-2 text-right text-slate-500">${c.total_calls}</td>
        <td class="px-4 py-2 text-right font-semibold ${rateClass}">${successRate}%</td>
      </tr>
    `;
  }).join('') || '<tr><td colspan="5" class="px-4 py-6 text-center text-slate-400">暂无消耗数据</td></tr>';
}
...
async function loadLogs(offset = 0) {
  ...
  try {
    // 如果是首次加载日志 Tab，同时加载筛选器选项
    const filterOptsEl = document.getElementById('logFilterKey');
    if (filterOptsEl && filterOptsEl.options.length <= 1) {
      await loadLogFilterOptions();
    }

    // 构建查询参数
    const params = new URLSearchParams({ limit: 100, offset });
    if (logFilters.key_id) params.set('key_id', logFilters.key_id);
    if (logFilters.model) params.set('model', logFilters.model);
    if (logFilters.status) params.set('status', logFilters.status);

    const data = await api(`/api/admin/usage-logs?${params.toString()}`);
    ...
    document.getElementById('logsFilteredCount').textContent = `共 ${data.total} 条匹配记录`;
  } catch (err) {
...
async function loadLogFilterOptions() {
  try {
    const opts = await api('/api/admin/log-filter-options');

    const keySelect = document.getElementById('logFilterKey');
    keySelect.innerHTML = '<option value="">全部 Access Key</option>' +
      opts.keys.map(k => `<option value="${k.id}">${esc(k.name || k.key_value)}</option>`).join('');

    const modelSelect = document.getElementById('logFilterModel');
    modelSelect.innerHTML = '<option value="">全部模型</option>' +
      opts.models.map(m => `<option value="${esc(m)}">${esc(m)}</option>`).join('');
  } catch (err) {
    console.warn('加载筛选选项失败:', err.message);
  }
}

// 🔍 应用筛选条件并重新加载
function applyLogFilters() {
  logFilters.key_id = document.getElementById('logFilterKey').value;
  logFilters.model = document.getElementById('logFilterModel').value;
  logFilters.status = document.getElementById('logFilterStatus').value;
  currentLogOffset = 0;
  loadLogs(0);
}
...
function resetLogFilters() {
  logFilters = { key_id: '', model: '', status: '' };
  document.getElementById('logFilterKey').value = '';
