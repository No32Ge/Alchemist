...
adminApp.get('/api/admin/usage-logs', (req, res) => {
  const db = getDB();
  const { limit = 100, offset = 0, key_id, model, status } = req.query;

  let whereClauses = [];
  let params = [];

  if (key_id) {
    whereClauses.push('ul.access_key_id = ?');
    params.push(Number(key_id));
  }
  if (model) {
    whereClauses.push('ul.model = ?');
    params.push(model);
  }
  if (status) {
    whereClauses.push('ul.status = ?');
    params.push(status);
  }
  ...
  res.json({ logs, total: totalRow.count });
});

// ---- 🔮 获取日志筛选器选项（去重后的 key + model 列表） ----
adminApp.get('/api/admin/log-filter-options', (req, res) => {
  const db = getDB();
  const keys = db.prepare(`
    SELECT DISTINCT ak.id, ak.name, ak.key_value
    FROM usage_logs ul
    JOIN access_keys ak ON ul.access_key_id = ak.id
    ORDER BY ak.name
  `).all();
  const models = db.prepare(`
  ...
  `).all();
  res.json({
    keys: keys.map(k => ({ id: k.id, name: k.name || k.key_value, key_value: k.key_value })),
...
// ---- 趋势数据：24小时 / 近7日 积分与Token消耗折线图 ----
adminApp.get('/api/admin/analytics/trend', (req, res) => {
  const db = getDB();
  const days = parseInt(req.query.days) || 7;

  // 按小时分桶（24小时模式）或按天分桶（7天模式）
  if (days <= 1) {
    // 24小时模式：最近24小时，按小时聚合
    const since = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
    const rows = db.prepare(`
      SELECT
        strftime('%Y-%m-%dT%H:00', created_at) as bucket,
        COALESCE(SUM(CASE WHEN status = 'success' THEN credits_used ELSE 0 END), 0) as credits,
        COALESCE(SUM(CASE WHEN status = 'success' THEN prompt_tokens ELSE 0 END), 0) as prompt_tokens,
        COALESCE(SUM(CASE WHEN status = 'success' THEN completion_tokens ELSE 0 END), 0) as completion_tokens,
      ...
      ORDER BY bucket
    `).all(since);
    res.json({ granularity: 'hourly', days: 1, buckets: rows });
  } else {
    // 多天模式：按天聚合
    ...
    const rows = db.prepare(`
      ...
      ORDER BY bucket
    `).all(since);
    res.json({ granularity: 'daily', days, buckets: rows });
  }
});

// ---- 模型使用比例（饼图） ----
adminApp.get('/api/admin/analytics/model-usage', (req, res) => {
  const db = getDB();
  const rows = db.prepare(`
    SELECT
      model,
      COUNT(*) as call_count,
      COALESCE(SUM(CASE WHEN status = 'success' THEN credits_used ELSE 0 END), 0) as total_credits,
...
// ---- 请求状态占比（环形图） ----
adminApp.get('/api/admin/analytics/status-ratio', (req, res) => {
  const db = getDB();
  const rows = db.prepare(`
...
// ---- 积分消耗大户 Top N ----
adminApp.get('/api/admin/analytics/top-consumers', (req, res) => {
  const db = getDB();
  ...
  const rows = db.prepare(`
    SELECT
      ...
      ak.name as key_name,
      ak.key_value,
      COALESCE(SUM(CASE WHEN ul.status = 'success' THEN ul.credits_used ELSE 0 END), 0) as total_credits,
      COUNT(*) as total_calls,
      SUM(CASE WHEN ul.status = 'success' THEN 1 ELSE 0 END) as success_calls,
      SUM(CASE WHEN ul.status = 'failed' THEN 1 ELSE 0 END) as failed_calls
    FROM usage_logs ul
    JOIN access_keys ak ON ul.access_key_id = ak.id
    GROUP BY ak.id
...
// ---- 成功率趋势（配合趋势接口使用的摘要） ----
adminApp.get('/api/admin/analytics/summary', (req, res) => {
  const db = getDB();
  ...
  res.json({
    today: { total: today.total, success: today.success, credits: today.credits, prompt_tokens: today.prompt_tokens, completion_tokens: today.completion_tokens },
    week: { total: week.total, success: week.success, credits: week.credits, prompt_tokens: week.prompt_tokens, completion_tokens: week.completion_tokens }
  });
});
